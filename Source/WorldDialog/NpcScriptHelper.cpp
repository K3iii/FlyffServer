...
static std::map< std::string, PFN > g_functions;
...
void InitFunctions()
{
	...
	{
		str = g_tables[i].name;
		g_functions.insert( std::make_pair( str, g_tables[i].pmf ) );
	}
}
...
{
	std::map< std::string, PFN >::iterator it = g_functions.find( szFunction );
...
void CNpcScript::RemoveItems(DWORD dwID, int nCount )
{
	Functions* pTable = m_pInfo->GetFunctions();
	pTable->RemoveItems( m_pInfo, dwID, nCount );
}
...
int  CNpcScript::GetItems( DWORD dwID )
{
	Functions* pTable = m_pInfo->GetFunctions();
	return pTable->GetItems( m_pInfo, dwID );
}
...
void CNpcScript::RemoveKey(DWORD szKey )
{
	LPCTSTR lpszKey = GetDialogText( szKey );
