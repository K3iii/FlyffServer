...
// global variable
///////////////////////////////////////////////////////////////////////
static std::map< std::string, int >	g_mapKey;
...
{
	AddKey(9);
	if (GetItems(II_SYS_SYS_QUE_DRIANCARGO) == 0)
	{
		AddCondKey(45, 11);
	}
	AddKey(10);
...
{
	AddKey(12);
	if (GetItems(II_SYS_SYS_QUE_LETTERRANKASHU) == 0)
	{
		AddCondKey(336, 39);
	}
	AddKey(10);
...
void CNpcScript::masa_daz_1()
{
	if (GetPlayerLvl() >= 40)
	{
		if (IsSetQuest(QUEST_INTHEDARK) == TRUE && GetQuestState(QUEST_INTHEDARK) == QS_END)
		{
			if (IsParty() == TRUE)
			{
				if (GetItems(II_SYS_SYS_QUE_RUBY01) >= 1 && GetItems(II_SYS_SYS_QUE_RUBY02) >= 1 && GetItems(II_SYS_SYS_QUE_RUBY03) >= 1)
				{
					...
					{
						RemoveItems(II_SYS_SYS_QUE_RUBY01, 1);
						RemoveItems(II_SYS_SYS_QUE_RUBY02, 1);
						RemoveItems(II_SYS_SYS_QUE_RUBY03, 1);
					}
				}
...
BOOL MakeKeyIndex()
{
	...
	for (int i = 0; i < n; ++i)
	{
		...
		if (szText[0] != '\0')
			g_mapKey.insert(std::make_pair(szText, nIndex));
		else
			return FALSE;
	}
	return TRUE;
...
{
	std::map< std::string, int >::iterator it = g_mapKey.find(szKey);
	if (it != g_mapKey.end())
		return it->second;
	else
		return -1;
