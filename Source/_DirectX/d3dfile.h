...
public:
    // Rendering
    HRESULT Render( LPDIRECT3DDEVICE9 pd3dDevice,BOOL bDrawOpaqueSubsets,BOOL bDrawAlphaSubsets);
		virtual void Render(LPDIRECT3DDEVICE9 pd3dDevice) { Render(pd3dDevice,TRUE,TRUE); }
    ...
    HRESULT Destroy();
		virtual HRESULT Load(LPDIRECT3DDEVICE9 pd3dDevice,TCHAR* lpszFileName) { return Create(pd3dDevice,lpszFileName); }
