...
void CCoupleHelper::PostItem(u_long idPlayer, const COUPLE_ITEM& ci, int nLevel)
{
	...
	sprintf(szMailTitle, prj.GetText(TID_GAME_COUPLE_ITEM_MAIL_TITLE));
	sprintf(szMailText, prj.GetText(TID_GAME_COUPLE_ITEM_MAIL_TEXT), nLevel);
	kItemElem.m_dwItemId = ci.nItem;
	kItemElem.m_nItemNum = ci.nNum;
	if (ci.nLife > 0)
	{
		kItemElem.m_dwKeepTime = ::time_null() + ci.nLife * 60;
	}
...
void CCoupleHelper::OnPropose(CAr& ar, DPID dpid)
{
	...
	if (nResult == 0 && m_pMgr->GetCouple(idProposer))
		nResult = TID_GAME_COUPLE_E06;
	if (nResult == 0 && m_pMgr->GetCouple(idTarget))
		nResult = TID_GAME_COUPLE_E01;
	if (nResult == 0 && !SetPropose(idProposer, time(NULL) + 3600))
		nResult = TID_GAME_COUPLE_E10;
