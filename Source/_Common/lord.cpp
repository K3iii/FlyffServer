...
#else
CLEComponent::CLEComponent(int t, u_long p, float e, float i, float pe)
#endif
:
nTick( t ),
idPlayer( p ),
...
#else
	if (ar.IsStoring())
		ar << nTick << idPlayer << fEFactor << fIFactor << fPFactor;
	else
	{
		ar >> nTick >> idPlayer >> fEFactor >> fIFactor >> fPFactor;
#ifdef __CLIENT
...
BOOL ILordEvent::Initialize( const char* szFile )
{
	...
	while( s.tok != FINISHED )
	{
		...
		else if (s.Token == _T("penya"))
		{
			...
			{
				__int64 iCost = s.GetInt64();
				m_vPFactor.push_back(make_pair(fRate, iCost));
				fRate = s.GetFloat();
			}
		}
#endif
...
#else
__int64 ILordEvent::GetCost(int iEEvent, int iIEvent, int iPEvent)
{
	if( iEEvent == 0 && iIEvent == 0 && iPEvent == 0)
		return 0;
	if( iEEvent >= (int)( m_vEFactor.size() ) || iIEvent >= (int)( m_vIFactor.size() ) || iPEvent >= (int)(m_vPFactor.size()))
		return 0;
	return m_vEFactor[iEEvent].second + m_vIFactor[iIEvent].second + m_vPFactor[iPEvent].second;
}
#endif
...
#ifdef __FL_UPDATE_LORDEVENT
float ILordEvent::GetPFactor(int iPEvent)
{
	if (iPEvent >= (int)(m_vPFactor.size()))
		return 1.0F;
	return m_vPFactor[iPEvent].first;
}
#endif
...
#else
void ILordEvent::AddComponent(u_long idPlayer, int iEEvent, int iIEvent, int iPEvent)
{
	if (iEEvent == 0 && iIEvent == 0 && iPEvent == 0)
	{
#ifdef __DBSERVER
	...
	CLEComponent* pComponent
		= new CLEComponent(eTick, idPlayer, m_vEFactor[iEEvent].first, m_vIFactor[iIEvent].first, m_vPFactor[iPEvent].first);
	AddComponent(pComponent);
}
#endif
...
#ifdef __FL_UPDATE_LORDEVENT
float ILordEvent::GetPFactor()
{
	float fPenyaFactor = 1.0F;
	for (VLEC::iterator i = m_vComponents.begin(); i != m_vComponents.end(); ++i)
		fPenyaFactor += (*i)->GetPFactor();
	return fPenyaFactor;
}
#endif
