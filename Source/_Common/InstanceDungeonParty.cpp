...
CInstanceDungeonSolo::CInstanceDungeonSolo()
	: CInstanceDungeonBase(IDTYPE_SOLO)
{
}
...
CInstanceDungeonSolo* CInstanceDungeonSolo::GetInstance()
{
	static CInstanceDungeonSolo sIDP;
	return &sIDP;
}

#ifdef __WORLDSERVER
BOOL CInstanceDungeonSolo::EnteranceDungeon(CUser* pUser, DWORD dwWorldId)
{
	return TeleportToDungeon(pUser, dwWorldId, pUser->m_idPlayer);
}

BOOL CInstanceDungeonSolo::LeaveDungeon(CUser* pUser, DWORD dwWorldId)
{
	return LeaveToOutside(pUser, dwWorldId, pUser->GetLayer());
}
#endif // __WORLDSERVER
