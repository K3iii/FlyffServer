...
CEventLua::~CEventLua()
{
	...
	m_mapMonsterId.clear();
	m_vecSpawnRegion.clear();
	m_mapSpawnedMonster.clear();
	m_vecEndSpawnEvent.clear();
#endif // __EVENTLUA_SPAWN
				...
				case OT_ITEM:
				{
					...
					if( pProp )
					{
						...
						if( pObj )
						{
							...
							( (CItem*)pObj )->m_dwDropTime	= g_tmCurrent + MIN( 1440 );
	...
	for( vector<BYTE>::iterator itVec=m_vecEndSpawnEvent.begin(); itVec!=m_vecEndSpawnEvent.end(); )
	{
		...
		for( map<OBJID, BYTE>::iterator itMSM=m_mapSpawnedMonster.begin(); itMSM!=m_mapSpawnedMonster.end(); )
		{
			if( itMSM->second == (*itVec) )
			{
				...
				nDelCount++;
				if( nDelCount == (100/m_vecEndSpawnEvent.size()) )
					break;
			}
			else
				itMSM++;
