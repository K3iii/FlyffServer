...
{
	if( vMin2.x > vMax1.x )		return 0;
	if( vMax2.x < vMin1.x )		return 0;
	if( vMin2.z > vMax1.z )		return 0;
	if( vMax2.z < vMin1.z )		return 0;
	if( vMin2.y > vMax1.y )		return 0;
	if( vMax2.y < vMin1.y )		return 0;
						 ...
						 const D3DXVECTOR3 &v1, const D3DXVECTOR3 &v2, D3DXVECTOR3 *pvIntersect )
{
	...
	D3DXVECTOR3	rv1, rv2;
	D3DXVec3TransformCoord( &rv1, &v1, &mInv );
	D3DXVec3TransformCoord( &rv2, &v2, &mInv );

	BOOL bResult = IsTouchAABB_Line( vMin, vMax, rv1, rv2, pvIntersect );
	if( bResult)
...
{
	D3DXVECTOR3 vInvN;
	vInvN.x = -vN.x;	vInvN.y = -vN.y;	vInvN.z = -vN.z;
	FLOAT	b = D3DXVec3Dot( &vDir, &vInvN );
