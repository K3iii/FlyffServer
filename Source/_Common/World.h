...
public:
	...
	u_long			m_dwObjNum;
	CCtrl*			m_apObject[MAX_DYNAMICOBJ];
...
public:
#ifdef __WORLDSERVER
	void	removeObjectAtProcessArray(CObj* pObj);
#endif
	...
	BOOL	IsFFA(void)
	{
		return(m_dwWorldID == WI_WORLD_FFA);
	}
#endif
#ifdef __FL_TDM
	BOOL	IsTDM(void)
	{
		return(m_dwWorldID == WI_WORLD_TDM);
	}
#endif
...
protected:
	...
	void			InitContinent( const char* filename );
	void			InitAfterCreatedPlayer( );
	void			StartEV( const ENVIR_INFO& kOld );
	BOOL			CheckInOutContinent( );
	void			MoveInContinent( const ENVIR_INFO& kInfo );
	void			MoveOutContinent( const string& oldname );
	void			HookUpdateLight( CLight* pLight );
