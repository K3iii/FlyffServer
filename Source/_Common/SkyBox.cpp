...
void CSkyBox::InitFall()
{
	...
	if (m_nWeather == WEATHER_SAKURA)
		strcpy(fileName, _T("sakura.bmp"));
	else if (m_nWeather == WEATHER_LEAF)
		strcpy(fileName, _T("leaf.bmp"));
	else
		strcpy(fileName, _T("sundisk.bmp"));

	if (FAILED(LoadTextureFromRes(g_Neuz.m_pd3dDevice, MakePath(DIR_WEATHER, fileName), &m_pSunTexture)))
		Error("LoadTextureFromRes Default");
	...
	for (int i = 0; i < (int)(m_nFall); i++)
	{
		...
		else
		{
			...
			int z = xRandom(50) - 25;
			m_vVelocity[i] = D3DXVECTOR3(float(x) / 800, float(y) / 200 - 0.025f, float(z) / 800);
		}
	}
}
void CSkyBox::Process()
{
	...
	if (m_pWorld)
	{
		...
		{
			if (CWorld::m_skyBox.GetWeather() != WEATHER_SNOW)
		...
		else if (m_nWeather == WEATHER_SNOW || m_nWeather == WEATHER_SAKURA || m_nWeather == WEATHER_LEAF)
		{
			for (int i = 0; i < (int)(m_nFall); i++)
			{
				FLOAT fheight = m_pWorld->GetLandHeight_Fast(m_vFall[i].x, m_vFall[i].z) - 1.0f;
