...
void CWndMapEx::OnInitialUpdate(void)
{
	...
	m_pLockedAreaTexture = CWndBase::m_textureMng.AddTexture(g_Neuz.m_pd3dDevice, MakePath(getPathTheme(g_Option.getTheme()), "WndMapBlind.dds"), 0xff000000);
		...
		for (int i = 0; i < g_Party.GetSizeofMember(); ++i)
		{
			if (g_pPlayer->m_idPlayer != g_Party.GetPlayerId(i))
			{
				if (g_Party.m_aMember[i].m_bRemove == FALSE)
				{
					...
					{
						CMover* pPlayerMover = prj.GetUserByID(g_Party.GetPlayerId(i));
						if (IsValidObj(pPlayerMover))
							g_Party.SetPos(i, pPlayerMover->GetPos());
	...
	if (&m_WndMenuUserMark == pWndBase)
	{
		...
		case ID_USER_MARK_MENU_CHANGE_NAME:
		{
			...
			if (pUserMarkPositionInfo)
			{
				...
				m_pWndUserMarkNameChanger->Initialize(this);
...
void CWndMapEx::OnDestroyChildWnd(CWndBase* pWndChild)
{
	if (pWndChild == m_pWndUserMarkNameChanger)
		SAFE_DELETE(m_pWndUserMarkNameChanger);
}
//-----------------------------------------------------------------------------
...
void CWndMapEx::ResetMapInformation(void)
{
	...
	for (MapComboBoxDataVector::iterator Iterator = pMapComboBoxDataVector->begin(); Iterator != pMapComboBoxDataVector->end(); ++Iterator)
	{
		...
		if (dwSelectedMapNameItemData == pMapComboBoxData->GetID())
		{
...
#ifdef __MAP_TORCH_SYSTEM
			if (g_WorldMng()->m_bIsIndoor == FALSE && !g_WorldMng()->IsWorldInstanceDungeon())
			{
				if (m_dwSelectedMapID >= MCD_OMINOUS01 && m_dwSelectedMapID <= MCD_WdHeaven05
					|| m_dwSelectedMapID == MCD_DARKRARTESIA
					)
				{
					if (g_pPlayer->HasBuffByIk3(IK3_MAPOPEN_DUNGEON))
						m_pMapTexture = CWndBase::m_textureMng.AddTexture(g_Neuz.m_pd3dDevice, MakePath(getPathTheme(g_Option.getTheme()), pMapComboBoxData->GetPictureFileName()), 0xff000000);
					else
						m_pMapTexture = CWndBase::m_textureMng.AddTexture(g_Neuz.m_pd3dDevice, MakePath(getPathTheme(g_Option.getTheme()), "WndMapBlind.dds"), 0xff000000);
				}
				else
				{
					m_pMapTexture = CWndBase::m_textureMng.AddTexture(g_Neuz.m_pd3dDevice, MakePath(getPathTheme(g_Option.getTheme()), pMapComboBoxData->GetPictureFileName()), 0xff000000);
			...
			{
				m_pMapTexture = CWndBase::m_textureMng.AddTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_THEME, pMapComboBoxData->GetPictureFileName()), 0xffffffff);
...
BYTE CWndMapEx::GetMapArea(const D3DXVECTOR3& vPlayerPosition)
{
	...
	{
		switch (g_pPlayer->GetWorld()->GetID())
		{
		case WI_DUNGEON_FL_MAS: byPlayerLocationID = DUNGEON_MASDUNGEON; break;
		case WI_DUNGEON_SA_TA:
		case WI_DUNGEON_SA_TA2: byPlayerLocationID = DUNGEON_IBLESS; break;
		case WI_DUNGEON_DA_DK: byPlayerLocationID = DUNGEON_DEKANES; break;
		case WI_DUNGEON_VOLCANE:
		case WI_DUNGEON_VOLCANERED:
		case WI_DUNGEON_VOLCANEYELLOW: byPlayerLocationID = DUNGEON_VOLCANE; break;

		case WI_INSTANCE_RUSTIA: byPlayerLocationID = DUNGEON_RUSTIA_1; break;
		case WI_INSTANCE_RUSTIA_1: byPlayerLocationID = DUNGEON_RUSTIA_2; break;
#if __VER >= 16
		case WI_INSTANCE_UPRESIA: byPlayerLocationID = DUNGEON_UPRESIA; break;
		case WI_INSTANCE_UPRESIA_1: byPlayerLocationID = DUNGEON_UPRESIA_1; break;
		case WI_INSTANCE_HERNEOS: byPlayerLocationID = DUNGEON_HERNEOS; break;
		case WI_INSTANCE_HERNEOS_1: byPlayerLocationID = DUNGEON_HERNEOS_1; break;
		case WI_INSTANCE_SANPRES: byPlayerLocationID = DUNGEON_SANPRES; break;
		case WI_INSTANCE_SANPRES_1: byPlayerLocationID = DUNGEON_SANPRES_1; break;
		case WI_INSTANCE_BEHAMAH: byPlayerLocationID = DUNGEON_BEHEMOTH; break;
		case WI_INSTANCE_KALGAS:
		case WI_INSTANCE_KALGAS_1:
		case WI_INSTANCE_KALGAS_2: byPlayerLocationID = DUNGEON_KALGAS; break;
#endif
		case WI_INSTANCE_OMINOUS: byPlayerLocationID = DUNGEON_OMINOUS01; break;
		case WI_INSTANCE_OMINOUS_1: byPlayerLocationID = DUNGEON_OMINOUS02; break;
		case WI_INSTANCE_DREADFULCAVE: byPlayerLocationID = DUNGEON_DREADFULCAVE; break;

		case WI_WORLD_CISLAND: byPlayerLocationID = CASHAREA_CORALICELAND; break;
		case WI_WORLD_KEBARAS: byPlayerLocationID = CASHAREA_ASRIA; break;
		case WI_WORLD_RARTESIA: byPlayerLocationID = CASHAREA_RARTESIA; break;
#if __VER >= 16
		case WI_WORLD_DARKRARTESIA: byPlayerLocationID = CASHAREA_DARKRARTESIA; break;
#endif

		case WI_WORLD_HEAVEN01: byPlayerLocationID = DUNGEON_WdHeaven01; break;
		case WI_WORLD_HEAVEN02: byPlayerLocationID = DUNGEON_WdHeaven02; break;
		case WI_WORLD_HEAVEN03: byPlayerLocationID = DUNGEON_WdHeaven03; break;
		case WI_WORLD_HEAVEN04: byPlayerLocationID = DUNGEON_WdHeaven04; break;
		case WI_WORLD_HEAVEN05: byPlayerLocationID = DUNGEON_WdHeaven05; break;

		default: byPlayerLocationID = CContinent::GetInstance()->GetContinent(vPlayerPosition); break;
		}
	}
	else
		byPlayerLocationID = CContinent::GetInstance()->GetContinent(vPlayerPosition);

	if (byPlayerLocationID == TOWN_FLARINENOSPLE || byPlayerLocationID == TOWN_SAINCITY
		|| byPlayerLocationID == TOWN_DARKEN || byPlayerLocationID == TOWN_ELIUN
		)
	{
	...
	for (int i = 0; i < 4; i++)
	{
		RectTown[i] = prj.m_MapInformationManager.FindRealPosition(i + 51);

		if (g_pPlayer->GetPos().x >= RectTown[i].left
			&& g_pPlayer->GetPos().x <= RectTown[i].right
			&& g_pPlayer->GetPos().z >= RectTown[i].top
			&& g_pPlayer->GetPos().z <= RectTown[i].bottom)
		{
			byPlayerLocationID = i + 51;
		}
	}
...
void CWndMapEx::OnLButtonDblClk(UINT nFlags, CPoint point)
{
	if (g_pPlayer == NULL)
		return;
	...
	if (g_pPlayer->IsAuthHigher(AUTH_GAMEMASTER))
	{
		...
		CString string;
		string.Format("/teleport %d %f %f", g_WorldMng()->m_dwWorldID, vReconvertedPosition.x, vReconvertedPosition.z);
