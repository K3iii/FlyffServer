...
****************************************************/
#define		MAX_MEMBER_LIST_DRAW	14
#define		MEMBER_LIST_HEIGHT		18
...
void CWndGuildTabMember::OnDraw(C2DRender* p2DRender)
{
	...
	for (i = m_nCurrentList; i < m_nCurrentList + MAX_MEMBER_LIST_DRAW; i++)
	{
		...
		{
			int nMasterIndex = 27;
			if (/*m_nLevel >= 60 && */pMember->nLevel < 70)
				nMasterIndex = 27;
	...
	if (pWndBase == &m_wndMenu)
	{
		...
		case MGI_GUILD_LEAVE:
		{
			...
			else
			{
				...
				{
					g_DPlay.SendRemoveGuildMember(m_aList[m_nSelect].idPlayer);
				}
			}
		}
		break;
...
void CWndGuildTabMember::UpdateData()
{
	...
	if (pGuild)
	{
		...
		for (map<u_long, CGuildMember*>::iterator i = pGuild->m_mapPMember.begin(); i != pGuild->m_mapPMember.end(); ++i)
		{
			...
			m_aList[m_nMax].nMemberLv = pMember->m_nMemberLv;
			...
			m_aList[m_nMax].nGiveGold = pMember->m_nGiveGold;
			m_aList[m_nMax].dwGivePxpCount = pMember->m_dwGivePxpCount;
