...
BOOL CWndGuildMerit::OnChildNotify( UINT message, UINT nID, LRESULT* pLResult )
{
	...
	case WIDC_BUTTON1:		// OK
		{
			CWndEdit *pWndEdit = (CWndEdit *)GetDlgItem( WIDC_EDIT1 );
			m_nPenya = atoi( pWndEdit->GetString() );
			if( m_nPenya < 0 )
				m_nPenya = 0;
			...
			if( pGuild )
			{
				...
				if( m_nPenya )
				{
					...
					if( cbResult == CONTRIBUTION_OK )
						g_DPlay.SendGuildContribution( 0, m_nPenya );
					pGuild->MeritResultMsg( cbResult );
				}
...
void CWndGuildMeritCtrl::OnDraw( C2DRender* p2DRender )
{
	...
	for( int i = pt.y; i < (int)( m_pItemContainer.size() ); i++ )
	{
		int x	= ( i - pt.x ) % 6;
		int y	= ( i - pt.y ) / 6;
		CItemElem* pItemBase	= m_pItemContainer[i];
		if( pItemBase )
		{
			...
			if( rectHittest.PtInRect( point ) )
			{
				...
				g_WndMng.PutToolTip_Item( pItemBase, point2, &rectHittest, APP_GUILD_MERIT1);
			}
			if( i == m_nCurSel )
