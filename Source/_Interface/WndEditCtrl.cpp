...
void CWndEdit::OnSetFocus(CWndBase* pOldWnd)
{
	...
	//#if !defined (__AIO) // temporary "fix", re-check in future
...
void CWndEdit::OnKeyEnd()
{
	if ((int)(m_dwOffset) < m_string.GetLength())
	{
		...
		{
			if (IsDBCSLeadByte(chr) && IsDBCSLeadByte(chr2))
			{
...
BOOL CWndEdit::IsYouMessage(UINT msg, WPARAM wparam, LPARAM lparam)
{
	...
	case WM_KEYDOWN:
	{
		...
		case VK_DOWN:
		{
			...
			if (m_pParentWnd->OnChildNotify(EN_DOWN, m_nIdWnd, (LRESULT*)this))
			{
				OnKeyEnd();
				CPoint ptCaret = OffsetToPoint(m_dwOffset, m_szCaret);
				SetCaretPos(ptCaret);
...
void CWndEdit::OnChar_(UINT nChar)
{
	...
	{
		if (PRIMARYLANGID(g_imeMgr.m_hkl) == LANG_CHINESE && SUBLANGID(g_imeMgr.m_langId) == SUBLANG_CHINESE_TRADITIONAL)
			m_stringBack = m_string;
	}
	else if (nChar == 1) // CTRL-A : select all
	{
	...
	else if (nChar == 22 && m_bEnableClipboard) // Ctrl-V : Paste
	{
		DeleteBlock();
	...
	else if (nChar != 9) // !tab
	{
		...
		if (!iscntrl(nChar))
		{
			...
			if (PRIMARYLANGID(g_imeMgr.m_hkl) == LANG_CHINESE && SUBLANGID(g_imeMgr.m_langId) == SUBLANG_CHINESE_TRADITIONAL && isgraph(nChar))
				m_stringBack = m_string;
