...
void CDPLoginSrvr::OnPreJoin(CAr& ar)
{
	...
	BOOL f = g_PlayerMng.AddPlayer(idPlayer, lpszPlayer, lpszAccount, dwAuthKey);	// lock
	if (!f)
	{
		Error("CDPLoginSrvr::OnPreJoin: ID %d, Acc %s, lpszPlayer %s", idPlayer, lpszAccount, lpszPlayer);

		CPlayer* pPlayer = g_PlayerMng.GetPlayer(idPlayer);
		if (pPlayer && lstrcmp(lpszAccount, pPlayer->lpszAccount) == 0)
		{
