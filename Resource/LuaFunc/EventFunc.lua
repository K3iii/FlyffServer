		...
		for j in pairs(tItem) do
			...
			if( (nRandom > 0) and (nTickCount >= tItem[j].TimeOut) and (tItem[j].nLevel <= nLevel) ) then
				...
				if( tItem[j].Skip == 0 ) then
					...
					nCount = nCount + 1
					TRACE( "Event.lua : GetItem() - Drop - "..tItem[j].ItemId..", "..nRandom.."��, Skip:"..tItem[j].Skip.." �ð���:"..(nHour-1).." ~ "..nHour )
				else
					tItem[j].Skip = tItem[j].Skip - 1
					TRACE( "Event.lua : GetItem() - Skip - "..tItem[j].ItemId..", ���� Skip:"..tItem[j].Skip.." �ð���:"..(nHour-1).." ~ "..nHour )
				end
			end
		end
	end
